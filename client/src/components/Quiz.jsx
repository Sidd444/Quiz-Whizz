...
const Quiz = ({ quiz, onDelete }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`http://localhost:5000/api/quizzes/${quiz._id}/submit`, { answers });
    ...
    return (
        <div className="ml-96 mt-4 border p-4 rounded shadow-lg">
            ...
            {score !== null && <p className="mt-4">Your score: {score}/{quiz.questions.length}</p>}
        </div>
    );
};
